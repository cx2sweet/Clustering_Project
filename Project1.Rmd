...
```{r}
answers <- read.csv("results_df.csv")
answers
```

```{r}
sapply(answers,max)
apply(answers[,c(2,4,7,10)],1,max)
which(answers$ARI.1 == 1)
which(answers$ARI2 == 1)
ari.astark <-answers[,c(2,4,7,10)]
heatmap(as.matrix(ari.astark),Rowv = NA,Colv = NA)
Dunn.cecap<- answers[,c(3,5,8,12)]
heatmap(as.matrix(Dunn.cecap),Rowv = NA,Colv = NA)
```
      ...
      for(skew1 in skew){
          for (skew2 in skew){
            ...
            if(k==12){
              ...
              labels <- stuff$cluster
              model1 = kmeans(data, nstart = 25, centers = 2)
              s.vals <- silhouette(x=model1$cluster,dist=dist(data)) 
              plot(s.vals,main="Silhouette Plot of Two Groups Solution",col="blue") 
              plot(data, col = model1$cluster)
