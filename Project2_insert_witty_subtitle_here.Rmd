...
arg2=list(mu=mu2,Sigma=s2,shape=skew2)
args=list(arg1,arg2)
stuff=rmmix(n=200,p=c(.5,.5),family = 'Skew.normal',arg=args, cluster = TRUE)
data<-stuff$y
labels <- stuff$cluster
plot(data, col = labels)
...
ls<-c()
for (i in 1:10){
  model <- meanShift(data, trainData = data, nNeighbors = i,iterations=25)
  #print(ARI(labels, model$assignment))
  ls<-c(ls,ARI(labels, model$assignment))
}
...
model <- meanShift(data, trainData = data,  nNeighbors = 3, iterations=25)
ARI(labels, model$assignment)

plot(data, col = model$assignment)

table(model$assignment,labels)
```
